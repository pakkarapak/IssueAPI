...
router.get('/', (req: Request, res: Response) => {
    mongodb.collection("company").find().toArray().then((data) => {  // find ได้ค่ามาเป็น Array
        res.json(data);
    });
});
...
router.get('/findById/:id', (req: Request, res: Response) => {
    ...
    mongodb.collection("company").findOne({ _id: id }) // findone การหามา 1 object ( _id: id = where )
        .then((data) => {
            res.json(data);
        });
});
